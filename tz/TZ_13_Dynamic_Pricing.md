...
| **Трудоёмкость** | 2 000 человеко-часов |
...
|----|-------------|---------------|--------|
| **TZ-11** | Парсинг цен конкурентов | IT / подрядчик | ⬜ |
| **P-01** | Аудит API САМО-тур | Логвинов Г.А. | ⬜ |
| — | Историч. данные продаж (2+ года) | Баландина О.А. | ⬜ |
| — | Данные загрузки рейсов | САМО-тур | ⬜ |
...
|---|--------|-----|
| | **Подготовка данных** | |
| 1 | Анализ исторических данных продаж (2+ года) | 60 |
| 2 | Сбор и очистка данных из САМО-тур | 80 |
| 3 | Интеграция данных из TZ-11 (цены конкурентов) | 40 |
| 4 | Подготовка датасета для обучения модели | 100 |
| 5 | Feature engineering (создание признаков) | 80 |
| | **Разработка ML-модели** | |
| 6 | Исследование и выбор алгоритма (XGBoost, нейросети) | 100 |
| 7 | Обучение базовой модели | 120 |
| 8 | Настройка гиперпараметров | 80 |
| 9 | Валидация модели на исторических данных | 80 |
| 10 | A/B тестирование (сравнение с текущим подходом) | 100 |
| | **Интеграция с САМО-тур** | |
| 11 | Анализ API САМО-тур для обновления цен | 40 |
| 12 | Модуль чтения данных о загрузке рейсов | 60 |
| 13 | Модуль автоматического обновления цен | 100 |
| 14 | Режим рекомендаций (с подтверждением тарификатора) | 80 |
| 15 | Режим полуавтоматический (авто с возможностью отката) | 60 |
| 16 | Режим автоматический (полностью без участия человека) | 80 |
| | **Дашборд управления** | |
| 17 | Дашборд: текущие цены vs рекомендации | 60 |
| 18 | Дашборд: загрузка рейсов и прогноз продаж | 60 |
| 19 | Дашборд: сравнение с конкурентами (данные TZ-11) | 40 |
| 20 | Отчёт: эффективность ценовых решений | 60 |
| 21 | Логирование всех решений (кто, когда, почему) | 40 |
| | **Тестирование** | |
| 22 | Тестирование на исторических данных (backtesting) | 80 |
| 23 | Пилотный запуск на 2-3 направлениях | 100 |
| 24 | Мониторинг результатов пилота (2-4 недели) | 60 |
| | **Внедрение** | |
| 25 | Масштабирование на все направления | 80 |
| 26 | Обучение тарификаторов работе с системой | 40 |
| 27 | Обучение руководства работе с дашбордами | 20 |
| 28 | Документация (техническая + пользовательская) | 60 |
| 29 | Сопровождение при запуске (4 недели) | 80 |
| | **ИТОГО** | **2 000** |

> **Примечание:** Смета носит оценочный характер для тендера. Финальная детализация — после выбора подрядчика и этапа проектирования. Требуется предварительное внедрение TZ-11 (парсинг цен конкурентов).
...
| # | Действие | Ответственный | Зависимость |
|---|----------|---------------|-------------|
| 1 | **Завершить TZ-11** (парсинг цен) | IT / подрядчик | **КРИТИЧНО** |
| 2 | Собрать исторические данные (2+ года) | Баландина О.А. | — |
| 3 | Провести аудит API САМО (P-01) | Логвинов Г.А. | — |
| 4 | Согласовать ТЗ с руководством | Баландина О.А. | — |
| 5 | Сформировать список подрядчиков | IT + Закупки | — |
| 6 | Рассылка RFP | Закупки | п.4, п.5 |
| 7 | Сбор предложений | Закупки | п.6 |
| 8 | Оценка и выбор | Комиссия | п.7 |
| 9 | Подписание контракта | Юристы | п.8 |
...
| **Трудоёмкость** | 2 000 человеко-часов |
| **Экономия времени** | 55 ч/мес |
| **Ожидаемый эффект** | +5-10% маржи, -50% «горения» мест |
